...
def update_U(R, I, a=1e-05):
    ...
    for i in range(R.shape[1]):
        Xi = I[~np.isnan(R[:,i])] #the selected training data set for user i
        yi = R[:,i][~np.isnan(R[:,i])]
        temp = ridge_regression(Xi, yi, a)
        U.append(temp)
...
def update_I(R, U, a=1e-05):
    #########################################
    ## INSERT YOUR CODE HERE (7 points)
    # I = train_item_i(extract_item_i(R, U), a)
    I = []
    for i in range(R.shape[0]):
        Xi = U[~np.isnan(R[i, :])]  # the selected training data set for user i
        yi = R[i, :][~np.isnan(R[i, :])]
        temp = train_item_i(Xi, yi, a)
        I.append(temp)
